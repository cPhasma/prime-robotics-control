...
// В этой аппаратной схеме PWM инверсный:
// PWM = 0   -> максимальная скорость
// PWM = 255 -> минимальная скорость / фактическая остановка
const int MOTOR_STOP_PWM = 255;
...
void setMotor(int ctrlPin, int pwmPin, const char* direction, int pwm) {
  ...
  if (strcmp(direction, "forward") == 0) {
    ...
    analogWrite(pwmPin, MOTOR_STOP_PWM);
  }
}
...
void initMotor() {
  ...
  analogWrite(ML_PWM, MOTOR_STOP_PWM);
  analogWrite(MR_PWM, MOTOR_STOP_PWM);
}

CmdResult handleMotor(JsonObject params) {
  int left_pwm = params["left_pwm"] | MOTOR_STOP_PWM;
  int right_pwm = params["right_pwm"] | MOTOR_STOP_PWM;
